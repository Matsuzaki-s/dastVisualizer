/*
 * @(#)CommandInterpreter.java	1.24 05/11/17
 *
...
import nd.com.sun.tools.example.debug.bdi.BreakpointSpec;
...
import nd.com.sun.tools.example.debug.bdi.ThreadGroupIterator;
import nd.com.sun.tools.example.debug.bdi.ThreadIterator;
import nd.com.sun.tools.example.debug.bdi.Utils;
import nd.com.sun.tools.example.debug.bdi.VMLaunchFailureException;
...
public class CommandInterpreter {
	...
	private void commandClasses() throws NoSessionException {
		List<ReferenceType> list = runtime.allClasses();
	...
	private int printThreadGroup(OutputSink out, ThreadGroupReference tg, int iThread) {
		...
		List<ThreadGroupReference> tglist = tg.threadGroups();
		for (int ig = 0; ig < tglist.size(); ig++) {
			ThreadGroupReference tg0 = tglist.get(ig);
	...
	private boolean doLoad(boolean suspended,
		...
		String token = context.getMainClassName() + "." + methodname;
	...
	private void listEventRequests() throws NoSessionException {
		...
		Iterator<EventRequestSpec> iter = runtime.eventRequestSpecs().iterator();
		if (!iter.hasNext()) {
	...
	private void commandClear(StringTokenizer t) throws NoSessionException {
		...
		if (bpSpec != null) {
			...
			if (!iter.hasNext()) {
				...
				for (Iterator<BreakpointSpec> it = toDelete.iterator(); it.hasNext();) {
					BreakpointSpec spec = it.next();
					runtime.delete(spec);
				}                    
			}
		} else {
	...
	private void commandLocals() throws NoSessionException {
		...
		for (Iterator<LocalVariable> it = vars.iterator(); it.hasNext(); ) {
