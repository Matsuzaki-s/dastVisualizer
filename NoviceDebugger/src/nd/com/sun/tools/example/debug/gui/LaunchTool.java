...
class LaunchTool {
    ...
    private Connector selectConnector() {
        ...
        for (Iterator<Connector> it = all.iterator(); it.hasNext(); ) {
    ...
    private void configureAndConnect(final Connector connector) {
        ...
        final ArrayList<ArgRep> argReps = new ArrayList<ArgRep>(args.size());
        for (Iterator<Argument> it = args.values().iterator(); it.hasNext(); ) {
            Object arg = it.next();
            ArgRep ar;
        ...
        JPanel buttonPanel = okCancel( dialog, new ActionListener() { 
            public void actionPerformed(ActionEvent event) { 
                for (Iterator<ArgRep> it = argReps.iterator(); it.hasNext(); ) {
