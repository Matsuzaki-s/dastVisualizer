/*
 * @(#)ChildSession.java	1.15 05/11/17
 *
...
class ChildSession extends Session {
	...
	public boolean attach() {
		...
		/*
		 * Create a Thread that will retrieve and display any output. Needs to
		 * be high priority, else debugger may exit before it can be displayed.
		 */

		// ### Rename InputWriter and OutputReader classes
		// ### Thread priorities cribbed from ttydebug. Think about them.

		OutputReader outputReader = new OutputReader("output reader", "output",
				out, output, diagnostics);
		outputReader.setPriority(Thread.MAX_PRIORITY - 1);
		outputReader.start();

		OutputReader errorReader = new OutputReader("error reader", "error",
		...
		// ### debug
		// System.out.println("IO after attach: "+ inputWriter + " " +
	...
	public void detach() {
		...
		// ### debug
		// System.out.println("IO before detach: "+ inputWriter + " " +
		// outputReader + " "+ errorReader);
	...
	private boolean connectToVMProcess() {
		...
		err = new BufferedReader(
				new InputStreamReader(process.getErrorStream()), 1);
		return true;
	}
	...
	private static class OutputReader extends Thread {
		...
		OutputReader(String threadName, String streamName,
				BufferedReader stream, OutputListener output,
				OutputListener diagnostics) {
			super(threadName);
		...
		public void run() {
			try {
				...
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						diagnostics.putString("IO error reading " + streamName
	...
	private static class InputWriter extends Thread {
		...
		/*
		 * public void quit() { //### Won't have much effect if blocked on
		 * input! running = false; }
		 */
