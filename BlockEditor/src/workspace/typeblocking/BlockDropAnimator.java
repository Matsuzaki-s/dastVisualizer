...
/**
 * The BlockDropAnimator has two function, to fly a block to it's location and
 * then attempt to link the child if possible. It takes in a RenderableBlock,
 * drags the block, and drops it if possible. This class uses a timer to assist
 * in the smooth animation of flight.
 */
public class BlockDropAnimator implements ActionListener {
	 ...
	 */
	public BlockDropAnimator(Point focusPoint, RenderableBlock childBlock,
	...
	public void actionPerformed(ActionEvent e) {
		if (childBlock.getLocation().distance(focusPoint) < 75) {
			...
			if (parentBlock != null && parentBlock.getBlockID() != null
					&& !parentBlock.getBlockID().equals(Block.NULL)) {
				BlockLink link = LinkFinderUtil.connectBlocks(
						Block.getBlock(childBlock.getBlockID()),
						Block.getBlock(parentBlock.getBlockID()));
				if (link == null) {
					...
					System.out.println(Block.getBlock(parentBlock.getBlockID())
							.getBlockLabel());
					link.connect();
					dropBlock(childBlock);

					Workspace.getInstance().notifyListeners(
							new WorkspaceEvent(RenderableBlock
									.getRenderableBlock(link.getPlugBlockID())
									.getParentWidget(), link,
									WorkspaceEvent.BLOCKS_CONNECTED));
					RenderableBlock.getRenderableBlock(link.getSocketBlockID())
							.moveConnectedBlocks();
					RenderableBlock.getRenderableBlock(link.getSocketBlockID())
							.repaintBlock();
					RenderableBlock.getRenderableBlock(link.getSocketBlockID())
			...
			//stop the timer
			timer.stop();
			if (Block.getBlock(childBlock.getBlockID()).getGenusName()
					.equals("number")) {
			...
			//check if focumanager's before parent is same as
			//the parent we have here and check if new focusblock is child block
			PageChangeEventManager.notifyListeners();
		} else {
			//childBlock.setLocation(focusPoint);
			// This needs to change if the parent block doesn't have any more sockets for children
			// Need to adjust focusPoint somehow.
			childBlock.setLocation((int) (focusPoint.getX() * 0.67)
					+ (int) (childBlock.getX() * 0.34),
					(int) (focusPoint.getY() * 0.67)
							+ (int) (childBlock.getY() * 0.34));
	 ...
	 * @param block
	 * 
	 * @requires block != null; block.blockID != null; block.blockID !=
	 *           Block.NULL workspace widget != null
	 */
	private static void dropBlock(RenderableBlock block) {
		if (block == null) {
			throw new RuntimeException(
					"Invariant Violated: child block was null");
		}
		Page p = Workspace.getInstance().getCurrentPage(block);
		if (p == null) {
			throw new RuntimeException(
					"Invariant Violated: child block was located on a null widget");
		}
