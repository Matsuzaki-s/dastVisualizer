...
public class EventThread extends Thread {
	...
	EventThread(VirtualMachine vm, String[] excludes, PrintWriter writer,
		...
		this.writer = writer;
		this.rf = rf;
		objm = new ObjectManager(rf.getClassDefinition());
	}
	 ...
	 */
	void setEventRequests(boolean watchFields) { // ここで必要なイベントを指定？
		...
		// mgr.modificationWatchpointRequest();
	...
	class ThreadTrace {
		...
		ThreadTrace(ThreadReference thread) {
			...
			/*
			 * List<ReferenceType> cls = vm.allClasses();
			 * 
			 * 
			 * for ( int i = 0; i < 10; ++i ) { println(cls.get(i).toString());
			 * List<ObjectReference> obl = cls.get(i).instances(0); for(int j =
			 * 0; j < obl.size(); ++j){ List<Field> fls = cls.get(i).fields();
			 * for(int k = 0; k < fls.size(); k++){
			 * println(fls.get(k).name().toString() +" = "
			 * +obl.get(j).getValue(fls.get(k))); } } }
			 */
		...
		void fieldWatchEvent(ModificationWatchpointEvent event) {
			...
			println("    " + object.toString() + field.name() + " = " + value);
			step();
		}
	...
	private void handleEvent(Event event) {
		if (event instanceof ExceptionEvent) {
			...
			writer.println("ModificationWatchpoint");
			fieldWatchEvent((ModificationWatchpointEvent) event);
		} else if (event instanceof MethodEntryEvent) {
			writer.println("MethodEntry");
			methodEntryEvent((MethodEntryEvent) event);
		} else if (event instanceof MethodExitEvent) {
			writer.println("methodExitEvent");
			methodExitEvent((MethodExitEvent) event);
		} else if (event instanceof StepEvent) {
			writer.println("StepEvent");
			stepEvent((StepEvent) event);
		} else if (event instanceof ThreadDeathEvent) {
			writer.println("ThredDeathEvent");
			threadDeathEvent((ThreadDeathEvent) event);
		} else if (event instanceof ClassPrepareEvent) {
			writer.println("ClassPrepare");
			classPrepareEvent((ClassPrepareEvent) event);
		} else if (event instanceof VMStartEvent) {
			writer.println("VMStartedEvent");
			vmStartEvent((VMStartEvent) event);
		} else if (event instanceof VMDeathEvent) {
			writer.println("VMDeathEvent");
	...
	/*
	 * private void step(EventSet eventSet){ vm.resume(); BufferedReader reader
	 * = new BufferedReader(new InputStreamReader(System.in)); try {
	 * reader.readLine(); } catch (IOException e) { // TODO Auto-generated catch
	 * block e.printStackTrace(); } eventSet.resume();
	 * 
	 * 
	 * }
	 */
