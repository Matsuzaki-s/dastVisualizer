...
public class EventThread extends Thread {
	...
	//追加
	//ModificationWatchpointRequest mwr = mgr.modificationWatchpointRequest();
	...
	ThreadTrace(ThreadReference thread) {
		...
		for ( int i = 0; i < 10; ++i ) {
			println(cls.get(i).toString());
			List<ObjectReference> obl = cls.get(i).instances(0);
			for(int j = 0; j < obl.size(); ++j){
				List<Field> fls = cls.get(i).fields();
				for(int k = 0; k < fls.size(); k++){
				println(fls.get(k).name().toString() +" = " +obl.get(j).getValue(fls.get(k)));
				}
			}
		}*/
    ...
    private void classPrepareEvent(ClassPrepareEvent event)  {
        ...
        EventRequestManager mgr = vm.eventRequestManager();
        if(rf.isTarget(event.referenceType().name())){
        	TargetClass.add(event.referenceType());
        }
   ...
   /* private void step(EventSet eventSet){
    	vm.resume();
    	BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
    	try {
			reader.readLine();
